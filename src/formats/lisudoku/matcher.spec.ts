...
test('detects lisudoku data string', async () => {
  const result = await matcher.run(LISUDOKU_DATA_STRING)
  expect(result).toEqual({
    matched: true,
    dataString: LISUDOKU_DATA_STRING,
    constraints: LISUDOKU_CONSTRAINTS,
...
test('detects lisudoku solver url with grid string', async () => {
  const result = await matcher.run(LISUDOKU_SOLVER_GRID_STRING)
  expect(result).toEqual({
    matched: true,
...
test('detects grid string', async () => {
  const result = await matcher.run(GRID_STRING)
  expect(result).toEqual({
    matched: true,
    dataString: GRID_STRING,
    constraints: LISUDOKU_GRID_STRING_CONSTRAINTS,
...
test('invalid lisudoku data should return error', async () => {
  const result = await matcher.run(LISUDOKU_SOLVER_INVALID_URL)
  expect(result).toEqual({
    matched: true,
    dataString: '1234',
...
test('invalid lisudoku url should not match', async () => {
  const result = await matcher.run('https://abc.xyz/solver?import=1234')
  expect(result).toEqual({
    matched: false,
  })
})
...
test('invalid lisudoku constraints schema should return error', async () => {
  ...
  const result = await matcher.run('https://lisudoku.xyz/solver?import=N4IgHiBcDMC+Q%3D%3D%3D')
  expect(result).toEqual({
    matched: true,
